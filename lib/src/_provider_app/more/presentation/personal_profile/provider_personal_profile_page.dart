...
import '../../../main_page/models/provider_main_page_tabs_enum.dart';
import '../../../main_page/observer/provider_main_page_observer.dart';
import '../../domain/use_cases/update_provider_profile_use_case.dart';
import 'provider_personal_profile_cubit.dart';

class ProviderPersonalProfilePage extends StatelessWidget {
  const ProviderPersonalProfilePage({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocSelector<ProviderPersonalProfileCubit, ProviderPersonalProfileState, Async<ProviderEntity>>(
        selector: (state) {
          return state.getDataState;
        },
        builder: (context, state) {
          final userData = state.data;
          if (state.isSuccess && userData != null) {
            return _PageBody(user: userData);
          } else if (state.isLoading) {
            return const SpinKitLoadingWidget();
          } else if (state.isFailure) {
            return AppFailWidget(onRetry: context.read<ProviderPersonalProfileCubit>().getData);
          }
...
class _PageBody extends StatefulWidget {
  const _PageBody({required this.user});

  final ProviderEntity user;
...
class __PageBodyState extends State<_PageBody> {
  ...
  void _onUpdateProfileSuccess() async {
    ...
    await Future.delayed(const Duration(milliseconds: 1500), () {
      ProviderMainPageUpdater.notifyOnChangedCallbacks(ProviderMainPageTabsEnum.more);
      if (mounted) {
  ...
  void initState() {
    super.initState();
    _avatorController = ValidatorFieldController<AttachmentEntity?>(initialValue: widget.user.avatar);
    _nameController = TextEditingController(text: widget.user.name);
  }

  @override
  Widget build(BuildContext context) {
    return BlocListener<ProviderPersonalProfileCubit, ProviderPersonalProfileState>(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Expanded(
                          flex: 3,
                          child: PhoneField(controller: TextEditingController(text: widget.user.mobile), readOnly: true),
                        ),
                        Expanded(
                          child: Padding(
                            ...
                            child: OutlinedButton(
                              onPressed: () {
                                UpdatePhonePage.show(context, phone: widget.user.mobile);
                              },
