...
part "provider_wallet_state.dart";

@Injectable()
class ProviderWalletCubit extends Cubit<ProviderWalletState> {
  ProviderWalletCubit(this._getWalletInfoUseCase, this._withdrawBalanceUseCase, this._getBalanceUseCase)
    : super(const ProviderWalletState.initial());
  ...
  @override
  void emit(ProviderWalletState state) {
    if (!isClosed) {
      super.emit(state);
    }
  }
